...
export default function CollectionPage() {
  ...
  const filtered = useMemo(() => filterAndSortProducts(products, handle, selectedSizes, selectedColors, sortBy), [products, handle, selectedSizes, selectedColors, sortBy]);
    ...
    <div className="min-h-screen" style={{ paddingTop: 80 }}>
      <div className="border-b border-[#E8E4DC] px-6 md:px-10 py-12 md:py-16 max-w-[1400px] mx-auto">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="text-[#8A8A82] text-xs font-light mt-5">{filtered.length} product{filtered.length !== 1 ? 's' : ''}</p>
        </motion.div>
      </div>

      <div className="max-w-[1400px] mx-auto px-6 md:px-10 py-8">
        <div className="flex items-center justify-between mb-8 gap-4">
          <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 text-[#1A1A18] hover:text-[#C9A96E] transition-colors">
            <SlidersHorizontal size={15} strokeWidth={1.5} />
            <span className="section-label text-[11px]">FILTER</span>
            {hasFilters && <span className="bg-[#C9A96E] text-[#1A1A18] text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{selectedSizes.length + selectedColors.length}</span>}
          </button>

          <div className="flex items-center gap-2">
            <span className="section-label text-[11px] text-[#8A8A82]">SORT:</span>
            <div className="relative">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="appearance-none bg-transparent border border-[#E8E4DC] px-4 py-2 text-xs font-medium text-[#1A1A18] pr-8 outline-none cursor-pointer hover:border-[#1A1A18] transition-colors">
        ...
        {showFilters && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} transition={{ duration: 0.3 }} className="border border-[#E8E4DC] p-6 mb-8 bg-[#FAFAF8]">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                <p className="section-label text-[10px] text-[#8A8A82] mb-3">SIZE</p>
                <div className="flex flex-wrap gap-2">
                  {allSizes.map((size) => (
                    <button key={size} onClick={() => toggleSize(size)} className="w-10 h-10 border text-xs font-medium transition-all" style={{ borderColor: selectedSizes.includes(size) ? '#1A1A18' : '#D1CCC3', background: selectedSizes.includes(size) ? '#1A1A18' : 'transparent', color: selectedSizes.includes(size) ? '#FAFAF8' : '#1A1A18' }}>{size}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {allColors.map((color) => (
                    <button key={color.name} onClick={() => toggleColor(color.name)} title={color.name} className="w-7 h-7 rounded-full border-2 transition-transform hover:scale-110" style={{ background: color.hex, borderColor: selectedColors.includes(color.name) ? '#1A1A18' : '#D1CCC3', transform: selectedColors.includes(color.name) ? 'scale(1.2)' : 'scale(1)' }} />
                  ))}
                </div>
              </div>
              ...
              {hasFilters && (
                <div className="flex items-end">
                  <button onClick={() => { setSelectedSizes([]); setSelectedColors([]); }} className="flex items-center gap-2 text-[#8A8A82] hover:text-[#1A1A18] transition-colors">
                    <X size={14} />
